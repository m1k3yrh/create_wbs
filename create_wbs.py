...
# Load configuration from local config.py file
		...
		self.story_pointed_work_items=self.load_config('Story Pointed Work Items')
...
def missing_points_report():
	if config.story_pointed_work_items:		
		missing_points=[row for row in filtered_data if row.type in config.story_pointed_work_items and (row.points==0 or row.points==None or row.points=='Unassigned')]		
		if missing_points!=[]:
			error_worksheet.append_warning("Warning: The following Work Items appear to not have Story Points assigned.  Velocity and Backlog values may be under-reported.")
			error_worksheet.append_list(missing_points)
